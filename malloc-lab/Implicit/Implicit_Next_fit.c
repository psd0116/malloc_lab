...
static void* find_fit(size_t asize)
{
    ...
    // 1. last_bp 다음 블록부터 힙의 끝까지 탐색
    // last_bp는 직전에 찾은 블록을 가리킨다.
    for (bp = NEXT_BLKP(last_bp); GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp))
    {
        if (!GET_ALLOC(HDRP(bp)) && (GET_SIZE(HDRP(bp)) >= asize))
        {
            last_bp = bp; // 찾은 위치를 다음 검색을 위해 저장
            return bp;
        }
    }

    // 2. 끝까지 찾지 못했을 경우 처음부터 다시 탐색
    for (bp = NEXT_BLKP(heap_listp); bp != NEXT_BLKP(last_bp); bp = NEXT_BLKP(bp))
    {
        if (!GET_ALLOC(HDRP(bp)) && (GET_SIZE(HDRP(bp)) >= asize))
        {
            last_bp = bp;
...
// Next-fit
// last_bp에서부터 에필로그까지 탐색을 하는데 끝까지 갔을때까지 없으면 다시 처음부터 탐색해야해
...
static void* coalesce(void *bp)
{
    ...
    // (버그 수정을 위해) 이전/다음 블록 포인터를 미리 저장합니다.
    ...
    if (prev_alloc && next_alloc) { /* Case 1 */
        ...
        size += GET_SIZE(HDRP(next_bp)); // (next_bp 변수 사용)
        PUT(HDRP(bp), PACK(size, 0));
        PUT(FTRP(bp), PACK(size, 0));
        
        // ★ (수정) last_bp가 '다음' 블록을 가리켰다면, '현재' 블록으로 옮김
        if (last_bp == next_bp)
            last_bp = bp;
    }
    else if (!prev_alloc && next_alloc) { /* Case 3 */
        size += GET_SIZE(HDRP(prev_bp)); // (prev_bp 변수 사용)
        PUT(FTRP(bp), PACK(size, 0));
        PUT(HDRP(prev_bp), PACK(size, 0));
        
        // ★ (수정) last_bp가 '현재' 블록을 가리켰다면, '이전' 블록으로 옮김
        if (last_bp == bp)
            last_bp = prev_bp;
        
        bp = prev_bp; // bp 반환값을 이전 블록으로 변경
    }
    else { /* Case 4 */
        ...
        // ★ (수정) last_bp가 '현재' 또는 '다음' 블록을 가리켰다면, '이전' 블록으로 옮김
        if (last_bp == bp || last_bp == next_bp)
            last_bp = prev_bp;
