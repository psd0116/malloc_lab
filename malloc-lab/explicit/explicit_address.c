...
static void insert_node(void *bp)
{
    ...
    char *curr = free_listp; // 리스트 헤드부터 검색 시작
    char *prev = NULL;      // 이전 노드 추적

    // 삽입 위치 찾기: bp의 주소보다 큰 노드를 만날 때까지 순회
    while (curr != NULL && (char *)bp > curr) {
        prev = curr;
        curr = GET_SUCC(curr);
    }

    // bp의 PRED, SUCC 설정
    PUT_PTR(SUCC_PTR(bp), curr); // bp -> succ = curr
    PUT_PTR(PRED_PTR(bp), prev); // bp -> pred = prev

    // 리스트 연결 갱신
    if (curr != NULL) { // Case 1: 리스트 중간 또는 맨 끝에 삽입
        PUT_PTR(PRED_PTR(curr), bp); // curr -> pred = bp
    }
    if (prev != NULL) { // Case 2: 리스트 중간 또는 맨 끝에 삽입 (head 아님)
        PUT_PTR(SUCC_PTR(prev), bp); // prev -> succ = bp
    } else { // Case 3: 리스트 맨 앞에 삽입 (bp가 새 head)
        free_listp = bp;
    }
}
